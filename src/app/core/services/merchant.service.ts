...
export interface MerchantSettings {
  ...
  loyaltyCardsDisabled?: boolean; // New: Toggle for disabling loyalty cards
  notificationsEnabled?: boolean;
  notificationTemplateWelcome?: string;
  notificationTemplateRemaining?: string;
  notificationTemplateRewardClose?: string;
  isLoyaltyPaused?: boolean; // New: Loyalty pause status
  loyaltyPausedUntil?: string | Date; // New: When pause expires
}
...
export class MerchantService {
  ...
  /**
   * Record wash with full details (uses scan-qr endpoint internally)
   */
  recordWash(washData: {
    customerId: string;
    ...
    // Use the scan-qr endpoint with ProcessWashRequest format
    // The backend expects customerQRCode, merchantId, amount, and serviceIds
    const processRequest = {
      customerQRCode: washData.customerQRCode,
      merchantId: washData.merchantId,
      amount: washData.price,
      serviceIds: [] // Backend expects list of service IDs, we can add wash type later
    };
    return this.apiService.post(`merchant/${washData.merchantId}/scan-qr`, processRequest);
  }
...
export interface MerchantRegistrationQR {
  qrCodeBase64: string; // Fixed to match backend property name
